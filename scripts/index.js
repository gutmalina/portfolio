...
letterList.forEach((letter, item) => {
  ...
  letter.style.setProperty(
    "animation",
    `loadingLetter 0.5s linear ${count}s forwards`
  );
});

const openHeaderMenu = () => {
  header.classList.toggle("header__visible");
  menuBtn.classList.toggle("btn__close_theme_white");
  menuLinks.forEach((menuLink) =>
    menuLink.addEventListener("click", closeHeaderMenu)
  );
};

const closeHeaderMenu = () => {
  header.classList.remove("header__visible");
  menuBtn.classList.remove("btn__close_theme_white");
  menuLinks.forEach((menuLink) =>
    menuLink.removeEventListener("click", closeHeaderMenu)
  );
};
...
background.addEventListener("mousemove", (e) => {
  ...
  const gradientBottom =
    ...
    " " +
    mousePos.y +
    ", transparent 5%, #1d1d1d 25%)";
  const gradientCenter =
    "radial-gradient(circle at " +
    mousePos.x +
    " " +
    mousePos.y +
    ", transparent, #1d1d1d 20%)";
  const gradientTop =
    "radial-gradient(circle at " +
    mousePos.x +
    " " +
    mousePos.y +
    ", transparent -5%, #1d1d1d 15%)";
...
menuBtn.addEventListener("mousedown", openHeaderMenu);
